...
func NewHostportManager(iptables utiliptables.Interface) HostPortManager {
	...
	if !h.conntrackFound {
		logrus.Errorf("NOK5G21-145: The binary conntrack is not installed, this can cause failures in network connection cleanup.")
	}
	return h
}
...
func (hm *hostportManager) Add(id string, podPortMapping *PodPortMapping, natInterfaceName string) (err error) {
	...
	if hm.execer != nil && hm.conntrackFound {
		logrus.Errorf("NOK5G21-145: Starting to delete udp conntrack entries: %v, isIPv6 - %v", conntrackPortsToRemove, isIPv6)
		for _, port := range conntrackPortsToRemove {
			err = conntrack.ClearEntriesForPort(hm.execer, port, isIPv6, v1.ProtocolUDP)
			if err != nil {
				logrus.Errorf("NOK5G21-145: Failed to clear udp conntrack for port %d, error: %v", port, err)
			}
		}
	}
	return nil
...
func (hm *hostportManager) syncIPTables(lines []byte) error {
	logrus.Errorf("NOK5G21-145: Restoring iptables rules: %s", lines)
	...
	if retErr != nil {
		for hp, socket := range ports {
			if err := socket.Close(); err != nil {
				logrus.Errorf("NOK5G21-145: Cannot clean up hostport %d for pod %s: %v", hp.port, getPodFullName(podPortMapping), err)
			}
		}
		return nil, retErr
	}
...
func (hm *hostportManager) closeHostports(hostportMappings []*PortMapping) error {
	...
	for _, pm := range hostportMappings {
		...
		if socket, ok := hm.hostPortMap[hp]; ok {
			logrus.Errorf("NOK5G21-145: Closing host port %s", hp.String())
			if err := socket.Close(); err != nil {
			...
			logrus.Errorf("NOK5G21-145: host port %s does not have an open socket", hp.String())
		}
	}
